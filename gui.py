...
class ResultsApp:
    ...
    def create_widgets(self):
        ...
        self.lb = tk.Listbox(frame, width=120, height=20)
        ...
        self.edit_btn = tk.Button(
            self.root, text="Editar Seleccionado", command=self.open_edit_popup
        )
    ...
    def populate_listbox(self):
        ...
        if selection and self.last_tests:
            selected_uid = self.last_tests[selection[0]].get("unique_id")

        self.lb.delete(0, tk.END)
        for row in self.last_tests:
            uid = row.get("unique_id")
            meta = self.meta.get(uid, {})
            display = (
                f"{row.get('test_id','')} | {row.get('date','')} {row.get('time','')} | "
                f"Res Máx: {row.get('max_resistance','')} | "
        ...
        if selected_uid:
            for i, row in enumerate(self.last_tests):
                if row.get("unique_id") == selected_uid:
    ...
    def open_edit_popup(self):
        ...
        name_entry.insert(0, self.meta.get(uid, {}).get("Name", ""))

        tk.Label(popup, text="Descripción:").grid(row=1, column=0)
        desc_entry = tk.Entry(popup, width=50)
        desc_entry.grid(row=1, column=1)
        desc_entry.insert(0, self.meta.get(uid, {}).get("Description", ""))
