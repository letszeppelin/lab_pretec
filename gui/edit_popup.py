...
def open_edit_popup(parent, unique_id, test_id, meta, refresh_callback):
    ...
    # --- Inicializar valor fck ---
    saved_fck = meta.get(unique_id, {}).get("fck", None)
    options_str = [str(x) for x in fck_options if x != "Otro"]

    if saved_fck is None:
        fck_var.set(21)
        fck_custom_entry.grid_remove()
    elif str(saved_fck) in options_str:
        fck_var.set(str(saved_fck))
        fck_custom_entry.grid_remove()
    else:
        fck_var.set("Otro")
        fck_custom_entry.delete(0, END)
        fck_custom_entry.insert(0, str(saved_fck))

    # --- Función para mostrar/ocultar "Otro" ---
    ...
    def copiar_todo():
        ...
        if "fck" in last_meta:
            val = last_meta["fck"]
            if str(val) in options_str:
                fck_var.set(str(val))
            else:
                fck_var.set("Otro")
                fck_custom_entry.delete(0, END)
                fck_custom_entry.insert(0, str(val))
    ...
    tb.Button(quick_frame, text="Copiar fecha", bootstyle="secondary", command=copiar_fecha).grid(row=0, column=0, padx=PAD_X, pady=(PAD_Y, 15))
    tb.Button(quick_frame, text="Copiar todo", bootstyle="info", command=copiar_todo).grid(row=0, column=1, padx=PAD_X, pady=(PAD_Y, 15))
    tb.Button(quick_frame, text="Vaciar todo", bootstyle="danger", command=vaciar_todo).grid(row=0, column=2, padx=PAD_X, pady=(PAD_Y, 15))
